...
% Load required data:
load([data_dir file_TM],'cond','TM_pval'); %JN editted from TM_pval2sided
...
for s_ind = 1:length(subplot_indices)
    ...
    for c_out = 1:selectedK
        for c_in = 1:selectedK
            if TM_pval(s_ind,c_out,c_in) < 0.05 && TM_pval(s_ind,c_out,c_in) > (0.05/selectedK) %JN edited from TM_pval2sided
                text(X(c_out,c_in),Y(c_out,c_in)+.22,'*','FontSize',16,'HorizontalAlignment','center','FontWeight','bold','Color','r');
            end
            if TM_pval(s_ind,c_out,c_in) < (0.05/selectedK) && TM_pval(s_ind,c_out,c_in) > (0.05/(selectedK*selectedK))%JN edited from TM_pval2sided
                text(X(c_out,c_in),Y(c_out,c_in)+.22,'*','FontSize',16,'HorizontalAlignment','center','FontWeight','bold','Color','g');
            end
            if TM_pval(s_ind,c_out,c_in) <= (0.05/(selectedK*selectedK))%JN edited from TM_pval2sided
